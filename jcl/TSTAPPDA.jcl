//TSTAPPDA JOB (A),'NSF APP PARK',CLASS=A,MSGCLASS=H,
//             MSGLEVEL=(1,1),NOTIFY=&SYSUID
//*
//* 40-CHK -- the PARKA arm: a batch client that dies WITH A REQUEST
//* OUTSTANDING, which is the only state in which the ADR-0040 client-death
//* guard has anything to protect.
//*
//* It listens on port 7799 and blocks in ACCEPT -- ubuf-free, so it reaches the
//* guard and the reply POST instead of faulting first (issue #80).  Nothing sends to that port, so
//* the request stays published and this task waits in the SVC routine on the
//* CSA reply ECB until an operator cancels it:
//*
//*     C TSTAPPDA
//*
//* THEN make one TCP connection to port 7799 to COMPLETE the parked request, which
//* is what drives the STC to the guard and the reply POST:
//*
//*     ssh mvsdev "nc -w2 192.168.200.1 7799"
//*
...
//* PREREQUISITE:  S NSFS
//*
//RUN     EXEC PGM=TSTAPPD,PARM='PARKA',REGION=8M
