...
*          exit uses to hand a pre-allocated QELEM to the executive
*          task
...
*  hand-rolled STM/BALR/USING seam breaks the cc370 C-runtime path
*  (@@CRTGET,
*  ABEND S0C6); see issue #8. The FUNHEAD entry name IS the 8-char
*  asm() alias
*  in include/nsfxq.h (NSFXINIT / NSFXPUSH / NSFXDRAN), character for
*  character
*  (PR #7).
*
*  RS-FORMAT OPERANDS (#5): the CS / LM operands are written D(B), NOT
*  D(,B).
*  as370 mis-assembles the empty-index RX form D(,B) on an RS
*  instruction with
*  base register 0 (cc370 issue #12). The RX-format L operands use
*  D(,B).
*
*  ABA safety: exactly one consumer, which removes work by swapping out
*  the
*  WHOLE chain at once (xq_drain); individual elements are never
*  popped, so a
*  node cannot be freed and re-pushed to reappear as a stale-but-equal
*  head.
*  Do NOT add a per-element pop.
*
*  cc370 linkage: args are a fullword list at R1 (arg0 at 0(R1), arg1
*  at
*  4(R1)); a pointer result returns in R15 via FUNEXIT RC=(Rn).
*
*  STATUS: assembles and links; the CS loop's RUNTIME on 3.8j is
*  validated by
*  the on-MVS TSTXQ suite at M0-6 (the host build swaps
*  src/nsfxq_host.c). The
*  entry convention is fixed now (issue #8); only the runtime stays
*  deferred.
*
         COPY  MVSMACS
         COPY  PDPTOP
