...
| NSFSTS | Statistics | 8 | — |
| NSFDEV / NSFCTCI / NSFLCS / NSFHOST | Devices & drivers (NSFDEV table + DEVOPS + optional DEVIO seam + NSFHOST host driver, M1-2; CTCI top half `asm/nsfctcio.asm` + channel/SVC 99 seam `src/nsfctci.c` M1-3; M1-4 codec `src/nsfctcif.c`, bottom half `src/nsfctcib.c`, host shims `src/nsfctcio_host.c`/`src/nsfctci_host.c` — completion path blocked on M1-4b, ADR-0022) | 9 | 200–299 |
