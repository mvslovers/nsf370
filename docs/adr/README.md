...
| 0046 | **Descriptor ownership at the request boundary.** A socket descriptor was not merely forgeable but **guessable in tens of attempts** from an unauthorised address space (one socket table, lowest free index, `(gen<<16)|idx`). Resolution now runs through **one** function, `nsfreq_sock_owned`, which **resolves and checks** so no caller can hold an unchecked socket; two callers (`req_socket`, and `sel_scan` — the wider door, one descriptor per SELECT mask item). The input is the **FLIH-captured caller identity, never `r->apptok`**: measured, the facade populates that field on only 3 of ~20 verbs, so comparing against it deadlocks every honest client. Scope is **per address space**; a foreign descriptor is **indistinguishable from an unknown one** (no existence oracle, no new errno). An enumeration guard makes a new, unclassified `sock_lookup` caller break the build | ADR-0046-descriptor-ownership-at-the-request-boundary.md |
| 0047 | **`ubuf` and `ulen` are transport-owned** — `ubuf` is an address in the caller's address space and `ulen` is a length in **bytes, for every verb**; a verb-specific meaning belongs in `sockdesc`/`p1`/`p2`/`p3`, which the transport never interprets. **The dual of ADR-0003**: that one says the protocol layer does not learn the transport, this one says the transport does not learn the verbs — and both defects of this family sat on the unstated half. `RQ_SELECT` alone encoded an **item** count (ADR-0035), so a cross-AS SELECT over N sockets crossed N bytes and was read as 8N; free in Phase 1, measured in Phase 2, and invisible to both existing tests because it lives in the seam between them. The facade multiplies, the dispatcher divides, a non-multiple is **refused `NSF_EINVAL`** rather than silently truncated, and `NSF_SIZE_ASSERT(NSFSELITEM, 8)` carries the size across the **two separate compilations** a host test cannot see. No new field; NSFRQE stays frozen at 64 B | ADR-0047-ubuf-ulen-are-transport-owned.md |
